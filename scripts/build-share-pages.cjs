...
const base = process.env.TNP_SITE_URL;
if (!base) throw new Error('Set TNP_SITE_URL to the deployed TNP Studio music root before regenerating pages.');
...
for (let i = 0; i < 13; i++) {
  ...
  html = html.replaceAll(`${base}share/14.html`, pageUrl)
    ...
    .replaceAll('English Version · 2026', `TRACK ${String(no).padStart(2,'0')} · TNP STUDIO`)
    .replaceAll('English Version', 'TNP Studio')
