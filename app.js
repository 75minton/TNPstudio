...
// TNP Studio build: 2026-09-24-tnp-v2 / assets: 20260924-tnp-v2
// 기본 커버 이미지 리소스입니다.
const defaultCover = new URL('./icons/tnp-studio-logo.png', new URL(document.currentScript?.src || './app.js', location.href)).href;

const APP_BUILD_VERSION = '2026-09-24-tnp-v2';
...
const STORAGE_SONGS_HASH_KEY = 'tnp_studio_songs_hash_v2';
const STORAGE_SONGS_SNAPSHOT_KEY = 'tnp_studio_songs_snapshot_v2';
const STORAGE_PLAYER_STATE_KEY = 'tnp_studio_player_state_v1_1';
const STORAGE_FAVORITES_KEY = 'tnp_studio_favorites_v1_1';
...
/* ?? IndexedDB ?ㅼ젙 ?? */
const IDB_NAME = 'tnp_studio_music';
const IDB_VERSION = 1;
...
function trackAnalyticsEvent(eventName, params = {}) {
  try {
    window.trackTnpEvent?.(eventName, params);
  } catch (err) {
...
function downloadTrack(song) {
  ...
  link.download = `${song.title || 'tnp-studio-track'}.mp3`;
...
async function shareTrack(song, index) {
  ...
  const text = `TNP Studio 추천곡: ${title}`;
...
function scheduleAutoAdvance(callback, { message = '잠시 후 다음 곡을 재생합니다' } = {}) {
  ...
  const gap = window.TnpLibrary?.gap() ?? TRACK_GAP_MS;
  showStatus(formatAutoAdvanceMessage(message, gap), { tone: 'info', duration: 0 });

  autoAdvanceCountdownTimer = window.setInterval(() => {
    const elapsed = Date.now() - startedAt;
    const remainingMs = Math.max(0, gap - elapsed);
...
function restoreStandaloneBackGuardEntry() {
  ...
  try {
    ...
    history.replaceState({ ...currentState, __TnpBackBase: true }, '', currentUrl);
    history.pushState({ __TnpBackTrap: true, at: Date.now() }, '', currentUrl);
...
function getHomeContentConfig() {
  const config = window.TNP_HOME_CONTENT && typeof window.TNP_HOME_CONTENT === 'object'
    ? window.TNP_HOME_CONTENT
    : {};
...
function setHomeTrackCard({ titleEl, artistEl, descEl, coverEl, playBtn, config, fallback }) {
  ...
  if (!song) {
    ...
    artistEl.textContent = 'TNP Studio';
    if (descEl) descEl.textContent = config?.description || '';
    coverEl.src = defaultCover;
    coverEl.alt = config?.title || 'TNP Studio cover';
...
const next = ({ autoAdvance = false } = {}) => {
  const queuedIndex = window.TnpLibrary?.takeNext();
  if (queuedIndex !== null && queuedIndex !== undefined) { loadTrack(queuedIndex, true); return; }
  if (!songs.length) return;
...
async function initializeApp() {
  ...
  window.dispatchEvent(new Event('tnp-songs-ready'));
  startSongsPolling();
  updateServiceWorkerIfPossible();
  checkForSongsUpdates();
}
